...
interface Stats { total: number; generated: number; skipped: Record<string, number> }
interface Endpoint {
    rawKey: string
    key: string
    methodName: string
...
async function main(): Promise<void> {
    ...
    for (const m of apiMethods) {
        ...
        if (!respIf) { incrSkip('no Response interface'); continue }
        const respWrap = respIf.getName()

        // RequestBody interface (任意)
        let reqWrap: string | undefined, mediaType: string | undefined, reqProps: string | undefined
        ...
        if (reqIf) {
            ...
            if (!mediaType) { incrSkip('no preferred mediaType'); continue }
        ...
        if (paramIf || paramTa) {
            ...
            paramProps = paramIf ? propsToDoc(paramIf) : paramTa?.getTypeNode()?.getText()
        }

        endpoints.push({
            rawKey, key: rawKey, methodName, httpVerb,
            reqWrap, respWrap, mediaType, paramType, reqProps, paramProps,
        })
        stats.generated++
    }
    ...
    project.createSourceFile(
        ...
        writer => {
            ...
            writer.writeLine(`import { ApiClient, Client as GeneratedClient, HttpMethod,`)
            endpoints.forEach(e => {
            ...
            writer.writeLine(`  T extends { 'application/json': { status?: infer S } } ? S : never;`)
            writer.writeLine('')
            endpoints.forEach(e => {
                writer.writeLine(`export type ${ucFirst(e.key)}Status = ExtractStatus<${e.respWrap}>;`)
            })
            writer.writeLine('')
            ...
            endpoints.forEach(e => {
                ...
                if (e.reqWrap && e.mediaType)
                    writer.writeLine(`export type ${nm}Request = NonNullable<${e.reqWrap}['${e.mediaType}']>;`)
                writer.writeLine(`export type ${nm}Response = NonNullable<${e.respWrap}['application/json']>;`)
                writer.writeLine('')
            })

            // ── MEDIA_TYPES / HTTP_VERBS / ENDPOINTS ──
            writer.writeLine(`const MEDIA_TYPES = {`)
            endpoints.forEach(e => writer.writeLine(`  ${e.key}: '${e.mediaType || ''}',`))
            writer.writeLine(`} as const;`)
            writer.writeLine('')
            writer.writeLine(`const HTTP_VERBS = {`)
            endpoints.forEach(e => writer.writeLine(`  ${e.key}: '${e.httpVerb}',`))
            writer.writeLine(`} as const;`)
            writer.writeLine('')
            ...
            endpoints.forEach(e => {
                ...
                writer.writeLine(`    extractStatus: (j:any) => { if (j.status == null) throw new Error('no status'); return j.status as any },`)
                writer.writeLine(`  },`)
            })
            writer.writeLine(`} as const;`)
            writer.writeLine('')
            ...
            writer.writeLine(`    const baseUrl = config.baseUrl ?? 'https://${HOST}';`)
            writer.writeLine(`    this.axios        = axios.create({ baseURL: baseUrl, ...config.axiosConfig });`)
            ...
            writer.writeLine(`      async request(method, url, headers, body, qp, opts = {}) {`)
            writer.writeLine(`        const { fullResponse, ...rest } = opts;`)
            writer.writeLine(`        const res = await axios.request({ method, url, headers, data: body, params: qp, ...rest });`)
            ...
            writer.writeLine(`    return async (body:any, headers={}, opts?:AxiosRequestConfig) => {`)
            writer.writeLine(`      const params = { parameter: headers, requestBody: { [MEDIA_TYPES[key]]: body } };`)
            writer.writeLine(`      const json = await (this.client as any)[fn](params, opts);`)
            ...
            writer.writeLine(`    const fn = ENDPOINTS[key].methodName;`)
            writer.writeLine(`    return async (query={}, headers={}, opts?:AxiosRequestConfig) => {`)
            writer.writeLine(`      const json = await (this.client as any)[fn]({ parameter: headers, query }, opts);`)
            writer.writeLine(`      return ENDPOINTS[key].extractStatus(json);`)
            writer.writeLine(`    };`)
            writer.writeLine(`  }`)
            writer.writeLine('')
            ...
            writer.writeLine(`    const fn = ENDPOINTS[key].methodName;`)
            writer.writeLine(`    return async (arg:any={}, headers={}, opts?:AxiosRequestConfig) => {`)
            writer.writeLine(`      const fullOpts = { ...(opts||{}), fullResponse:true };`)
            writer.writeLine(`      if (HTTP_VERBS[key] === 'get') {`)
            writer.writeLine(`        return (this.client as any)[fn]({ parameter: headers, query: arg }, fullOpts);`)
            writer.writeLine(`      } else {`)
            writer.writeLine(`        const params = { parameter: headers, requestBody: { [MEDIA_TYPES[key]]: arg } };`)
            writer.writeLine(`        return (this.client as any)[fn](params, fullOpts);`)
            writer.writeLine(`      }`)
            writer.writeLine(`    };`)
            writer.writeLine(`  }`)
            writer.writeLine('')
            ...
            endpoints.forEach(ep => {
                ...
                writer.writeLine('   * ```ts')
                if (httpVerb === 'get') {
                    writer.writeLine(`   * const res = await api.${key}(${paramType ? name + 'Params' : '{}'});`)
                } else {
                ...
                if (httpVerb === 'get') {
                    writer.writeLine(`    query?: ${paramType ? name + 'Params' : 'Record<string, any>'},`)
                } else {
                    writer.writeLine(`    body:  ${reqWrap && mediaType ? name + 'Request' : 'any'},`)
                }
                ...
                // ─── full-response JSDoc ───
                writer.writeLine('  /**')
                writer.writeLine(`   * ### ${httpVerb.toUpperCase()} /${endpointUrl} (full response)`)
                if (mediaType) writer.writeLine(`   * **Content-Type**: \`${mediaType}\``)
                writer.writeLine('   *')
                if (httpVerb === 'get') {
                    writer.writeLine(`   * @param query        ${queryDoc} URL クエリパラメータ (任意)`)
                ...
                writer.writeLine('   * @param extraHeaders 追加ヘッダー (任意)')
                writer.writeLine('   * @param axiosOpts    Axios オプション (任意)')
                writer.writeLine(`   * @returns ${fullRt} full axios response を返します`)
                writer.writeLine('   * @throws AxiosError ネットワーク／HTTP エラー')
                writer.writeLine('   * @example')
                writer.writeLine('   * ```ts')
                if (httpVerb === 'get') {
                    writer.writeLine(`   * const full = await api.${key}Full(${paramType ? name + 'Params' : '{}'});`)
                } else {
                    writer.writeLine(`   * const full = await api.${key}Full(${bodyDoc});`)
                }
    ...
    outFile.getInterfaces().forEach(i => {
        if (i.getMembers().length === 0) {
            i.addIndexSignature({ keyName: 'k', keyType: 'string', returnType: 'unknown' })
        }
    })
...
function toPascal(s: string) { return s.replace(/[^a-z0-9]+/gi, ' ').split(' ').map(ucFirst).join('') }
...
function propsToDoc(iface: import('ts-morph').InterfaceDeclaration) {
    return iface.getProperties().map(p => {
        const nn = p.getNameNode()
        const name = Node.isStringLiteral(nn) ? nn.getLiteralValue() : p.getName()
