...
export const build = async (endpoint: string): Promise<void> => {
  ...
  const scriptsDir = path.resolve(process.cwd(), 'scripts', 'generate')
  const customGen = path.join(scriptsDir, `${endpoint}.ts`)
  const commonGen = path.join(scriptsDir, 'generate-common.ts')

  if (fs.existsSync(customGen)) {
    console.log(`🔄 Found custom generator for “${endpoint}”`)
    console.log(`   yarn ts ${customGen} --host=${endpoint}`)
    await shell(`yarn ts ${customGen} --host=${endpoint}`)

  } else if (fs.existsSync(commonGen)) {
    console.log(`🔄 No custom generator for “${endpoint}”, running common generator…`)
    console.log(`   yarn ts ${commonGen} --host=${endpoint}`)
    await shell(`yarn ts ${commonGen} --host=${endpoint}`)

  } else {
    console.warn(`⚠️  Generator not found for “${endpoint}”, and common generator is missing.`)
    process.exit(1)
  }
